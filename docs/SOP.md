...
| `google_analytics_id` | Your Google Analytics 4 "Measurement ID" (looks like `G-XXXXXXXXXX`, from [analytics.google.com](https://analytics.google.com) → Admin → Data Streams → your web stream). Ships blank (`""`) — the site adds **zero** tracking scripts until you set this. Set it to `""` (not `null`) to turn tracking back off. |
