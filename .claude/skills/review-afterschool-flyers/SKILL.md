...
description: Check config/afterschool-programs.json for entries flagged needs_review, read their actual flyer, and fill in the real program details. Used both interactively and by the headless daily automation in scripts/flyer-review/.
---
...
`scripts/flyer-review/run.sh`'s scheduled systemd timer (see
...
new/removed/changed flyers in `assets/flyers/before-after-school/` —
see `.claude/skills/add-afterschool-program/`. What it *can't* do is
read a flyer to write its real program details; that's this skill's
whole job.
...
5. For each flagged entry, open its flyer —
   `assets/flyers/before-after-school/<flyer_filename>` (or ask an
   agent to) — and actually read what it says. Fill in real values for `name`,
...
7. `git add config/afterschool-programs.json pages/`, commit with a
   ...
   step needed (matches how the automated sync itself already commits
   directly to main).
